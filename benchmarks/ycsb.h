...
public:
  ...
  static PROMISE(rc_t) TxnInsert(bench_worker *w) {
    MARK_REFERENCED(w);
    RETURN {RC_TRUE};
  }
  static PROMISE(rc_t) TxnRead(bench_worker *w) {
    RETURN AWAIT static_cast<ycsb_worker *>(w)->txn_read();
  }
  ...
  static PROMISE(rc_t) TxnUpdate(bench_worker *w) {
    MARK_REFERENCED(w);
    RETURN {RC_TRUE};
  }
  static PROMISE(rc_t) TxnScan(bench_worker *w) {
    MARK_REFERENCED(w);
    RETURN {RC_TRUE};
  }
  static PROMISE(rc_t) TxnRMW(bench_worker *w) {
    RETURN AWAIT static_cast<ycsb_worker *>(w)->txn_rmw();
  }
  ...
  PROMISE(rc_t) txn_read();
  rc_t txn_read_amac();
  rc_t txn_read_coro();
  PROMISE(rc_t) txn_rmw();
