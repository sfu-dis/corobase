...
The majority of contentions occur among the analytic transaction, trade-result and market-feed transaction. The analytic transaction takes 10\% out of the workload mix. % TODO. new workload mix, footnote?
...
To vary contention degree, we change the size of a customer account group to be scanned by the analytic transaction. \figref{fig-tpce-scalability}(left) shows commit rates with 24 worker threads under varying contention degree from 5\% to 20\%. The X-axis indicates the size of the target customer account group.  %SILO is slightly faster than ERMIA-SI at the small(5\%) contention. As we increase contention degree, we can observe that ERMIA starts to outperform SILO and the performance gap gets larger; ERMIA has double commit rates along with half aborts at the 10\% query range. %ERMIA-SSI delivers ??\% higher throughput than SILO, even with the expensive serializability guarantee cost.
