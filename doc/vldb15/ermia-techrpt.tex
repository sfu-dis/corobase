...
\usepackage[
  ...
  pdfauthor={Kangnyeon Kim, Tianzheng Wang, Ryan Johnson, Ippokratis Pandis},
  pdfsubject={},
...
\newcommand{\labeledfigurerelay}[2][]{%
  \begin{figure}[t]%
    \centering%
    \includegraphics[width=\columnwidth]{\thelabeledfigure}%
    \ifthenelse{\equal{#1}{}}%
               {\caption{#2}}% then
               {\caption[#1]{#2}}% else
...
\begin{document}
\title{\thepapertitle}
\subtitle{\sf \fontsize{11}{11} \selectfont University of Toronto Technical Report (CSRG-623)}
\numberofauthors{1}
\author{
\alignauthor
Kangnyeon Kim
...
\input{ermia-abstract}
\input{ermia-intro}
\input{ermia-design}
\input{ermia-eval}
\input{ermia-related}
\input{ermia-conclusions}
