...
% already provide quite high performance, especially for the workloads they are optimized for (short-running partitionable transactions with small read/write footprints). Given that pessimistic CC is significantly more robust, the designer may seriously consider taking the hit and losing some of the 
...
Finally, there are also some sort of esoteric reasons, detailed in \secref{design:oid}, for using indirection arrays: space management becomes easier, and they admit a convenient analogue to cache-friendly compact index structures such as CSB+Trees \cite{RaoR00}. These benefits do not justify adopting indirection, but they become convenient to use once the indirection layer is in place for other reasons.
