...
# Load the results
data = MyData("../silo-microbench-results/rand-100K.csv", header_str="Reads", delimiter=' ');
...
## Get the entries for:
writes = (1, 10, 100, 1000, 10000)
#attrs = ('c^:', 'rd-', 'yo--', 'gs-.', 'b<-', 'c>-')
attrs = ('b^-', 'rd-', 'yo--', 'gs-.', 'b<-')

def drawLinesPayload(ax, write, ymax=500, showLegend=False):
    ...
    ratio = double(write)/double(1000);
...
plots0001 = drawLinesPayload(ax0001, 1)
ax0001.set_ylabel('Throughput (KTps)', fontsize=9)

plots001 = drawLinesPayload(ax001, 10)
plots01 = drawLinesPayload(ax01, 100, showLegend=True)
plots1 = drawLinesPayload(ax1, 1000, 100)
plots10 = drawLinesPayload(ax10, 10000, 10)
