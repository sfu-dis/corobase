...
public:
  ...
  static rc_t TxnNewOrder(bench_worker *w, void *param) {
    ...
    return static_cast<tpcc_dia_cmdlog_redoer *>(w)->txn_new_order((uint)wh);
  }

  rc_t txn_delivery(uint32_t warehouse_id);
  static rc_t TxnDelivery(bench_worker *w, void *param) {
    uint64_t wh = (uint64_t)param;
    return static_cast<tpcc_dia_cmdlog_redoer *>(w)->txn_delivery((uint)wh);
  }

  rc_t txn_payment(uint32_t warehouse_id);
  static rc_t TxnPayment(bench_worker *w, void *param) {
    uint64_t wh = (uint64_t)param;
    return static_cast<tpcc_dia_cmdlog_redoer *>(w)->txn_payment((uint)wh);
  }
...
public:
  ...
  static rc_t TxnNewOrder(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_new_order();
  }

  rc_t txn_delivery();

  static rc_t TxnDelivery(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_delivery();
  }

  rc_t txn_credit_check();
  static rc_t TxnCreditCheck(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_credit_check();
  }

  rc_t txn_payment();

  static rc_t TxnPayment(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_payment();
  }

  rc_t txn_order_status();

  static rc_t TxnOrderStatus(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_order_status();
  }

  rc_t txn_stock_level();

  static rc_t TxnStockLevel(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_stock_level();
  }

  rc_t txn_microbench_random();

  static rc_t TxnMicroBenchRandom(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_microbench_random();
  }

  rc_t txn_query2();

  static rc_t TxnQuery2(bench_worker *w) {
    return static_cast<tpcc_dia_worker *>(w)->txn_query2();
  }
    ...
    for (uint i = 0; i < 62; i++) {
      ...
      ((ermia::DecoupledMasstreeIndex *)tbl_nation(1))
          ->SendInsert(txn, rc, Encode(str(Size(k)), k),
                       Encode(str(Size(v)), v), &oid, &tuple, 0);
      ALWAYS_ASSERT(tuple);
      ((ermia::DecoupledMasstreeIndex *)tbl_nation(1))
...
protected:
  virtual void load() {
    ...
    for (uint i = 1; i <= NumWarehouses(); i++) {
      ...
      ermia::OID oid = 0;
      ((ermia::DecoupledMasstreeIndex *)tbl_warehouse(i))
          ->SendGet(txn, rc, Encode(str(Size(k)), k), &oid, 0);
      ((ermia::DecoupledMasstreeIndex *)tbl_warehouse(i))
        ...
        for (uint j = i + 1; j <= iend; j++) {
          ...
          ((ermia::DecoupledMasstreeIndex *)tbl_stock_data(w))
              ->SendInsert(txn, rc, Encode(str(Size(k_data)), k_data),
                           Encode(str(Size(v_data)), v_data), &oid, &tuple, 0);
          ALWAYS_ASSERT(tuple);
          ((ermia::DecoupledMasstreeIndex *)tbl_stock_data(w))
        ...
        for (uint c = 1; c <= NumCustomersPerDistrict();) {
          ...
          ((ermia::DecoupledMasstreeIndex *)tbl_oorder(w))
              ->SendInsert(txn, rc, Encode(str(Size(k_oo)), k_oo),
                           Encode(str(sz), v_oo), &v_oo_oid, &tuple, 0);
          ALWAYS_ASSERT(tuple);
          ((ermia::DecoupledMasstreeIndex *)tbl_oorder(w))
          ...
          if (c >= 2101) {
            ...
            ((ermia::DecoupledMasstreeIndex *)tbl_new_order(w))
                ->SendInsert(txn, rc, Encode(str(Size(k_no)), k_no),
                             Encode(str(sz), v_no), &oid, &tuple, 0);
            ALWAYS_ASSERT(tuple);
            ((ermia::DecoupledMasstreeIndex *)tbl_new_order(w))
          ...
          for (uint l = 1; l <= uint(v_oo.o_ol_cnt); l++) {
            ...
            ((ermia::DecoupledMasstreeIndex *)tbl_order_line(w))
                ->SendInsert(txn, rc, Encode(str(Size(k_ol)), k_ol),
                             Encode(str(sz), v_ol), &oid, &tuple, 0);
            ALWAYS_ASSERT(tuple);
            ((ermia::DecoupledMasstreeIndex *)tbl_order_line(w))
...
rc_t tpcc_dia_worker::txn_new_order() {
...
rc_t tpcc_dia_worker::txn_delivery() {
  const uint warehouse_id = pick_wh(r);
...
rc_t tpcc_dia_worker::txn_payment() {
  const uint warehouse_id = pick_wh(r);
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
  uint customerDistrictID, customerWarehouseID;
  if (likely(g_disable_xpartition_txn || NumWarehouses() == 1 ||
...
rc_t tpcc_dia_worker::txn_microbench_random() {
  ermia::transaction *txn = db->NewTransaction(0, arena, txn_buf());
  ermia::scoped_str_arena s_arena(arena);
...
private:
  ...
  static void RegisterTable(ermia::Engine *db, const char *name,
    ...
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      ...
      db->CreateMasstreeTable(name, true, primary_idx_name);
    }
  }
...
protected:
  virtual std::vector<bench_loader *> make_loaders() {
    ...
    ret.push_back(
        new tpcc_dia_district_loader(129856349, db, open_tables, partitions));
    if (ermia::config::parallel_loading) {
...
void tpcc_dia_do_test(ermia::Engine *db, int argc, char **argv) {
  // parse options
  optind = 1;
  ...
  if (ermia::config::verbose) {
    ...
    if (g_wh_temperature) {
      ...
      for (uint i = 0; i < tpcc_dia_worker::hot_whs.size(); i++)
...
rc_t tpcc_dia_cmdlog_redoer::txn_new_order(uint warehouse_id) {
  const uint districtID = RandomNumber(r, 1, 10);
  const uint customerID = GetCustomerId(r);
  const uint numItems = RandomNumber(r, 5, 15);
  uint itemIDs[15], supplierWarehouseIDs[15], orderQuantities[15];
...
rc_t tpcc_dia_cmdlog_redoer::txn_payment(uint warehouse_id) {
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
  uint customerDistrictID, customerWarehouseID;
  if (likely(g_disable_xpartition_txn || NumWarehouses() == 1 ||
             RandomNumber(r, 1, 100) <= 85)) {
...
rc_t tpcc_dia_cmdlog_redoer::txn_delivery(uint warehouse_id) {
  const uint o_carrier_id = RandomNumber(r, 1, NumDistrictsPerWarehouse());
  const uint32_t ts = GetCurrentTimeMillis();
  ...
  for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
    ...
    dia_new_order_scan_callback dia_new_order_c(s_arena.get());
    {
      ...
      ((ermia::DecoupledMasstreeIndex *)tbl_new_order(warehouse_id))
          ->RecvScan(txn, rc, dia_new_order_c);
    }
    TryCatch(rc);
    ...
    dia_static_limit_callback<15> dia_c(
        ...
        false); // never more than 15 order_lines per order
    ((ermia::DecoupledMasstreeIndex *)tbl_order_line(warehouse_id))
        ->SendScan(txn, rc, Encode(str(Size(k_oo_0)), k_oo_0),
