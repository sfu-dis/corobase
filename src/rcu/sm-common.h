...
struct fat_ptr {
     ...
     */
    static
    fat_ptr make(void *ptr, uint8_t sz_code, uint16_t flags=0) {
        union {
            void *v;
            intptr_t n;
        } u = {ptr};
    ...
    static
    fat_ptr make(uintptr_t n, uint8_t sz_code, uint16_t flags=0) {
    ...
    bool operator==(fat_ptr const &other) const {
 ...
 */
struct LSN {
    static LSN make(uintptr_t val, int segnum, uint8_t size_code=INVALID_SIZE_CODE) {
 ...
 */
size_t os_snprintf(char *buf, size_t size, char const *fmt, ...) __attribute__((format(printf,3,4)));
