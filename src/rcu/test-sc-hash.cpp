...
/* A way to track mean and variance of a large sample accurately with O(1) space.
...
struct obj {
    int x;
    bool operator==(obj const &other) const { eqcnt++; return x == other.x; }
    bool operator==(int ox) const { eqcnt++; return x == ox; }
};
...
int main() {
    ...
    size_t count=0;
    for (auto __attribute__((unused)) x : hset->hset)
        count++;
    ...
    for (auto &h : hset) {
        ...
        DEFER(free(name));
        ASSERT(not status);
        printf("\t%40s: %5.1f +/- %4.1f at cost %.1f/%.1f hash/comparisons per insert\n",
               name, h.stats.m, sqrt(h.stats.v), cost.first/total_sz, cost.second/total_sz);
    }


    eqcnt = 0;
    for (auto &h : hset)
        h.hset._hash.count = 0;
    ...
    for (auto &h : hset) {
        ...
        printf("\t%40s: cost %.1f/%.1f hash/comparisons per search\n",
               name, cost.first/total_sz, cost.second/total_sz);
    }
