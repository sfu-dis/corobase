...
int main() {
    ...
    printf("FYI: min/max representable values: %zd %zd\n",
            decode_size(0x00), decode_size(0xfe));
    ...
    int code;
    if ( (code=encode_size(0)) )
        printf("\tOops! Zero encodes to %02x\n", code);
    ...
    for (int i=1; i < 255; i++) {
        ...
        int b = decode_size(i);
        if (a >= b)
            printf("\tOops! %02x < %02x -> %d >= %d\n", i-1, i, a, b);
    }

    printf("Check whether any multiples of powers of two are not represented exactly (expect 7)...\n");
    for (size_t i=2; decode_size(encode_size(i)) == i; i *= 2) {
        for (int j=2; j < 32 and encode_size(j*i) != 0xff; j++) {
            if (decode_size(encode_size(j*i)) != j*i) 
                printf("\t%d * %zd = %zd -> %zd\n", j, i, j*i, decode_size(encode_size(j*i)));
        }
    }
}
