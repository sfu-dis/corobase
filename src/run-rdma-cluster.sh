...
run() {
  ...
  ./run-cluster.sh SI $t 10 $t $logbuf_mb $read_view_stat tpcc_org tpccr \
    "-group_commit -group_commit_size_mb=$group_commit_size_mb -chkpt_interval=1000000 -node_memory_gb=19 -log_ship_by_rdma -fake_log_write -wait_for_backups -num_backups=$num_backups -persist_policy=sync -read_view_stat_interval_ms=$read_view_ms -read_view_stat_file=$read_view_stat" \
