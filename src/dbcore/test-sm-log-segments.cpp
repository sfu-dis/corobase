...
#define P(msg, ...)                                        \
  ...
  std::_Rb_tree<uint64_t, log_segment_desc, keysel, std::less<uint64_t>,
                std::allocator<log_segment_desc> > segments;
  struct keyless {
  ...
  while (n < nmax) {
    ...
    for (auto x : pending) {
      ...
      if (not rval.first) {
        ...
        P("=> dead");
        if (verbose) {
          results[x] = std::make_pair("dead", -1);
        }
        ...
        if (it == segments.end())
          ...
          ASSERT(rval.first->segment_number == it->segment_number and
                 rval.first->end_offset == it->end_offset);
        ...
        if (rval.second) {
          ...
          if (verbose) {
            results[x] = std::make_pair("straddle", buf_offset);
          }
          ...
          if (verbose) {
            results[x] = std::make_pair("normal", buf_offset);
          }
  ...
  for (auto it : results) {
    if ((int64_t)it.second.second < 0)
      P("[%zd, %zd) -> ---- %s", it.first.first, it.first.second,
        it.second.first);
    else
      P("[%zd, %zd) -> %zd %s", it.first.first, it.first.second,
