...
#define P(msg, ...) do { if (verbose) fprintf(stderr, msg "\n", ##__VA_ARGS__); } while (0)
    ...
    while (n < nmax) {
        ...
        for (auto x : pending) {
            ...
            if (not rval.sid) {
                // dead zone
                P("=> dead");
                if (verbose) { results[x] = std::make_tuple("dead", -1, -1); }
                ...
                if (it == segments.end())
                    it = segments.insert(std::make_pair(rval.sid->start_offset, *rval.sid)).first;
                else
                    ASSERT(rval.sid->segnum == it->second.segnum
                           and rval.sid->end_offset == it->second.end_offset);
                ...
                if (not rval.full_size) {
                    ...
                    if (verbose) { results[x] = std::make_tuple("straddle", buf_offset, rval.next_lsn.offset()); }
                    ASSERT(it->second.end_offset < x.second+SKIP_LOG_SIZE);
                }
                else {
                    // normal
                    P("=> normal @%zd nxt=%zd", buf_offset, rval.next_lsn.offset());
                    if (verbose) { results[x] = std::make_tuple("normal", buf_offset, rval.next_lsn.offset()); }
    ...
    for (auto it : results) {
        ...
        if ((int64_t) here < 0)
            ...
            P("[%zd, %zd) -> %zd %s %zd", it.first.first, it.first.second, here, nm, next);
    }
}
int main() {
    rcu_register();
