...
struct sm_allocator {
    ...
    static_assert(is_aligned(sizeof(thread_cache), 64),
                  "Go fix thread_cache::N");
     ...
     */
    static constexpr
    size_t l3_alloc_size(uint32_t end_word=L3_MAX_WORDS) {
        return OFFSETOF(sm_allocator, l3_words[end_word]);
    }

    static constexpr
    size_t max_alloc_size() {
        return l3_alloc_size();
    };
    ...
    */
    ...
    static
    void destroy(sm_allocator* alloc);
...
static_assert(is_aligned(sm_allocator::l2_alloc_size(), sm_allocator::ASSUMED_PAGE_SIZE),
