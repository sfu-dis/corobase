...
RCU::rcu_quiesce()
{
    ...
    if (in_flight.erase(self) and in_flight.empty()) {
        ...
        for (rcu_active_list::iterator it=rcu_active.begin(); it != rcu_active.end(); ++it) {
    ...
    if (old_cooling.size())
        fprintf(stderr, "Thread %ld freed %d dead objects\n", (long) (uintptr_t) self, freed);
}

void __attribute__((noinline))
RCU::rcu_free(void const* p)
{
