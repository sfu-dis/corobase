        ...
        *req->rc = rc_t{RC_INVALID};
        ...
        case Request::kTypeGet:
          if (insert_ok || rc._val != RC_INVALID) {
            ...
            ASSERT((!insert_ok && rc._val != RC_INVALID) ||
                   (insert_ok && oid > 0 && rc._val == RC_TRUE));
          } else {
            // Haven't done any insert or read
            req->index->GetOID(*req->key, rc, req->transaction->GetXIDContext(),
                               oid);
  ...
  while (true) {
    ...
    for (auto &r : coalesced_requests) {
      ...
      case Request::kTypeGet:
        coroutines.push_back(
            new ermia::dia::generator<bool>(req->index->coro_GetOID(
                *req->key, tls_rcs[count], t->GetXIDContext(), *req->oid_ptr)));
        break;
      case Request::kTypeInsert:
        // Need to use the request's real OID
        coroutines.push_back(
