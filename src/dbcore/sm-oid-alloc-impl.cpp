...
void 
sm_allocator::destroy(sm_allocator *alloc)
{
    alloc->~sm_allocator();
}
...
sm_allocator::sm_allocator(dynarray &&self)
    : head(std::move(self))
{
}

void
sm_allocator::init_l2()
{
...
#ifndef NDEBUG
        l2_maps[i] = sparse_bitset{{{9191,8282,7373,6464,4646,3737,2828,1919}}};
#endif
    }
...
void
sm_allocator::init_l3(uint32_t end_word)
{
    ...
    if (head.l3_capacity < end_word) {
        ...
        if (not end_word) {
            ...
            // shrinking L3
            if (head.l3_scan_hand >= head.l3_capacity)
                head.l3_scan_hand = 0;
        }
    }
...
size_t
sm_allocator::alloc_size()
{
    if (l3_valid())
        return l3_alloc_size(head.l3_capacity);

    return l2_valid()? l2_alloc_size() : l1_alloc_size();
}

OID
sm_allocator::propose_capacity(size_t align)
{
...
bool
sm_allocator::fill_cache(thread_cache *tc)
{
    // careful of overrun
 ...
 drain_l1:
    ...
    if (head.l2_loose_oids + head.l3_loose_oids >= capacity_bump) {
        if (head.l2_loose_oids) {
            ...
            if (left > 0 and head.l3_loose_oids)
                left = _scavenge_l3(left);
...
bool
sm_allocator::drain_cache(thread_cache *tc, uint32_t target)
{
    if (not target)
        target = tc->nentries;

    // do we need to drain L1? is there an L2 to drain to?
    if (head.l1_size + tc->nentries > L1_CAPACITY)
...
int32_t
sm_allocator::_fill_from_l1(thread_cache *tc, uint32_t target)
{
...
int32_t
sm_allocator::_drain_l1(uint32_t target)
{
...
int32_t
sm_allocator::_scan_l2(uint32_t n, Filter const &f, Sink const &s)
{
    ...
    while (target > 0 and end_rejects < head.l2_size) {
        ...
        if (f(base)) {
            ...
            uint16_t bdx = x;
            for (auto offset : l2_maps[bdx].as_array())
                s(base+offset);
...
int32_t
sm_allocator::_fill_from_l2(thread_cache *tc, uint32_t target)
{
    // safe to access L2? also worthwhile?
    if (not l2_valid() or not head.l2_size)
        return target;
...
template <typename Filter, typename Sink>
int32_t
sm_allocator::_scavenge_l2(uint32_t n, Filter const &f, Sink const &s)
{
...
int32_t
sm_allocator::_drain_l2(uint32_t n)
{
...
template <typename Filter, typename Sink>
int32_t
sm_allocator::_scavenge_l3(uint32_t n, Filter const &f, Sink const &s)
{
...
int32_t
sm_allocator::_scavenge_l3(uint32_t n)
{
    if (not l3_valid())
        return 0;
...
sm_allocator::sanity_check()
{
    ...
    std::sort(values.begin(), values.end());
    {
        ...
        DIE_IF(dup != values.end(),
               "Duplicate value found in L1: %u", *dup);
    }
    ...
    for (auto it : enumerate(l2_assignments)) {
        ...
        if (it.second) {
            for (auto offset : l2_maps[it.second])
                values2.push_back(base + offset);
    ...
    std::sort(bitmaps.begin(), bitmaps.end());
    {
        ...
        DIE_IF(dup != bitmaps.end(),
               "One bitmap assigned to 2+ swaths: %u", uint32_t(*dup));
    }
    ...
    for (size_t i=0; i < head.l2_size; i++) {
        ...
        for (auto offset : l2_maps[bidx])
            values2.push_back(base + offset);
    }
    std::sort(bitmaps2.begin(), bitmaps2.end());
    {
        auto dup = std::adjacent_find(bitmaps2.begin(), bitmaps2.end());
        DIE_IF(dup != bitmaps2.end(),
               "One full bitmap found twice in L2: %u", uint32_t(*dup));
    }
    ...
    std::sort(bitmaps.begin(), bitmaps.end());
    {
        auto dup = std::adjacent_find(bitmaps.begin(), bitmaps.end());
        DIE_IF(dup != bitmaps.end(),
               "Full bitmap still assigned to a swath: %u", uint32_t(*dup));
    }
    ...
    std::sort(bitmaps2.begin(), bitmaps2.end());
    {
        auto dup = std::adjacent_find(bitmaps2.begin(), bitmaps2.end());
        DIE_IF(dup != bitmaps2.end(),
               "Same bitmap marked as idle 2+ times: %u", uint32_t(*dup));
    }
    ...
    std::sort(bitmaps.begin(), bitmaps.end());
    {
        auto dup = std::adjacent_find(bitmaps.begin(), bitmaps.end());
        DIE_IF(dup != bitmaps.end(),
               "Same bitmap marked as idle and in use: %u", uint32_t(*dup));
    }
    ...
    */
    std::sort(values2.begin(), values2.end());
    {
        ...
        DIE_IF(dup != values2.end(),
               "Duplicate value found in L2: %u", *dup);
    }
    ...
    std::sort(values.begin(), values.end());
    {
        auto dup = std::adjacent_find(values.begin(), values.end()); 
        DIE_IF(dup != values.end(),
               "Duplicate value found in L1/L2: %u", *dup);
    }
