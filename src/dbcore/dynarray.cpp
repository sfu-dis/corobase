...
size_t
dynarray::max_size() {
    return size_t(UINT32_MAX) << page_bits();
}
...
dynarray::dynarray(size_t capacity, size_t size)
    : _size(0)
{
...
dynarray::dynarray(dynarray &&victim) noexcept
    : dynarray()
{
...
void
swap(dynarray &a, dynarray &b) noexcept
{
    using std::swap;
...
dynarray::size() const
{
    return size_t(_size) << page_bits();
}

size_t
dynarray::capacity() const
{
    return size_t(_capacity) << page_bits();
}

void
dynarray::truncate(size_t new_size)
{
...
void
dynarray::resize(size_t new_size)
{
    // round up to the nearest page boundary
    new_size = align_up(new_size, page_size());
...
void
dynarray::_adjust_mapping(size_t begin, size_t end,
                          size_t new_size, bool make_readable)
{
