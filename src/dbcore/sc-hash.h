...
struct sc_hash_set {
    ...
    struct cmpeq {
        ...
        operator()(T const &t, K const &k) const { return t == k; }
    };
    
    typedef T value_type;
    typedef Hash hash_function;
    typedef typename std::conditional<std::is_same<void,Equals>::value, cmpeq, Equals>::type equals_function;
    ...
    static constexpr
    bool IS_MOVABLE = (std::is_move_constructible<T>::value
                       and std::is_move_assignable<T>::value);
...
public:
     ...
     */
    sc_hash_set(typename std::enable_if<std::is_default_constructible<hash_function>::value, dummy>::type=dummy{})
    {
        objzero(_presence);
    }
    
    template <typename H>
    sc_hash_set(H &&hash, typename std::enable_if<std::is_convertible<H, hash_function>::value, dummy>::type dummy=dummy{})
    ...
    size_t size() {
        size_t rval = 0;
        for (auto x : _presence)
            rval += __builtin_popcountll(x);
        return rval;
    }
    ...
    std::pair<iterator, int>
    find_and_emplace(Key const &x, Args && ...args) {
        auto h = _hash(x);
        {
    ...
    template <typename... Args>
    value_type *
    _init_value(size_t pos, Args && ...args)
    {
    ...
    value_type *
    _get_value(size_t pos)
    {
        return (value_type*) _buckets[pos].data;
    }

    void
    _destroy_value(size_t pos) {
        if (not std::is_trivially_destructible<value_type>::value)
            _get_value(pos)->~value_type();
    }

    std::pair<bitmap_word *, bitmap_word>
    _presence_info(size_t pos)
    {
        return std::make_pair(&_presence[pos/W_BITS],
                              bitmap_word(1) << (pos % W_BITS));
    }
