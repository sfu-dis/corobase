 ...
 * pools for reuse. During each succeeded update operation, the underlying
 *thread
 * will traverse the updated version chain to try garbage collecting versions
 *that
 * are no needed any more (decided by the epoch manager). Objects GC'ed from
 *version
 * chains will be put again to the TLS free objects pool. It is the update
 *threads
 ...
 *find
 * an object of the requested size in this pool, instead of the TLS bump
 *allocator.
 * If the free object pool is empty, we continue with the bump allocator; if the
 *bump
 * allocator also doesn't have free memory, we ask the central per-socket
 *reserved
 ...
 *thread
 * is GC-ing a version chain, it doesn't need to traverse too deep into the
 *chain.
 * If the epoch advances too slowly, i.e., it's possible then during an epoch a
 *tuple
 * is updated many times - very long chain - and yet the thread still cannot GC
 *the
 * versions because they belong to the same, relatively still new epoch. This
 *will
 * waste cycles and delay memory reclaimation as we're traversing a long chain
 *but
