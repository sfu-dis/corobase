...
struct sm_log_alloc_mgr {
  ...
  // most-recently
  // committed/aborted transaction's log lsn offset. This array together is used
  // to
  // replace the rcu-slist as a result of the decoupling of getting an LSN and
  // the
  // accounting of log allocation objects (in an rcu-slist). Using the rcu-slist
  // we
  // have to use **at least** one CAS per transaction and there's no bounded
  // number
  // of steps for a thread to successfully get an LSN offset. Tests on a 32-core
  // (4 sockets) and 60-core (4 sockets), and a 240-core (16 sockets) machines
  // show
  ...
  // the
  // rcu-slist term), which is the earliest LSN offset/transaction that has
  // written
  // to the log buffer. Note the discrepancy here: in both the rcu-slist and
  // atomic
  // fetch-and-add schemes, we might have holes in the log buffer. So we need to
  // find
  ...
  // problem because there is no coordination between threads on the same
  // socket.
  // That will direct us back to use CAS again which is what we wanted to avoid.
  //
  // The solution is borrowed from shore-mt's dlog's passive group commit, which
  // solves
  // a similar problem in NVM+dlog settings. Each thread will put its just
  // committed/aborted
  // lsn offset (corresponds to the log allocation block in the rcu-slist
  // scheme) to its
  // own "tls" place, and the log flusher just scans all these tls places, then
  // flush up
  ...
  // So smallest_tls_lsn_offset() returns the "durable lsn" (possibly still in
  // the
  // log buffer). The daemon is only responsible for flushing, ie making room in
  // the
