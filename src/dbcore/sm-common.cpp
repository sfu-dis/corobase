...
os_asprintf(char const *fmt, ...)
{
    ...
    THROW_IF (err < 0, os_error, errno, "Unable to format string template: %s", fmt);
    return msg;
}

int
os_open(char const *path, int flags)
{
...
size_t
os_pwrite(int fd, char const *buf, size_t bufsz, off_t offset) {
    size_t n = 0;
    while (n < bufsz) {
        ...
        THROW_IF (m < 0, os_error, errno,
                  "Error writing %zd bytes to file at offset %zd",
                  bufsz, offset);
        n += m;
    }
    return n;
...
size_t
os_pread(int fd, char *buf, size_t bufsz, off_t offset) {
    size_t n = 0;
    while (n < bufsz) {
        ...
        if (not m)
            break;
        THROW_IF (m < 0, os_error, errno,
                  "Error reading %zd bytes from file at offset %zd",
                  bufsz, offset);
        n += m;
    }
    return n;
...
void
os_truncate(char const *path, size_t size)
{
    int err = truncate(path, size);
    THROW_IF(err, os_error, errno, "Error truncating file %s to %zd bytes", path, size);
}

void
os_truncateat(int dfd, char const *path, size_t size)
{
...
void
os_renameat(int fromfd, char const *from, int tofd, char const *to)
{
...
void
os_unlinkat(int dfd, char const *fname, int flags)
{
...
size_t
os_snprintf(char *dest, size_t size, char const *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    DEFER(va_end(ap));
    ...
    if (size)
        dest[size-1] = 0; // in case user doesn't test return value!
    return n;
}
...
{
    THROW_IF(not _d, os_error, errno, "Unable to open/create directory: %s", dname);
}
...
void
dirent_iterator::iterator::operator++() {
    errno = 0;
    _dent = readdir(_d);
    if (not _dent) {
...
dirent_iterator::iterator
dirent_iterator::begin() {
    if (used)
        rewinddir(_d);
