...
transaction::transaction(uint64_t flags, str_arena &sa)
  ...
  if (config::is_backup_srv()) {
    ...
    xc = ctx;
    RCU::rcu_enter();
    xc->begin = volatile_read(*rep::global_persisted_lsn_ptr);
    ASSERT(xc->begin);
  } else {
...
namespace {
inline const char *transaction_state_to_cstr(txn_state state) {
  switch (state) {
    case TXN_ACTIVE:
      return "TXN_ACTIVE";
    case TXN_ABRTD:
...
rc_t transaction::parallel_ssn_commit() {
  ...
  try_get_successor:
    ...
    if (successor_clsn.asi_type() == fat_ptr::ASI_LOG) {
      ...
      while (not successor_end) {
        ...
        ALWAYS_ASSERT(successor_state == TXN_CMMTD or
                      successor_state == TXN_COMMITTING);
        successor_end = s;
      }
      if (should_continue) {
        continue;
  ...
  for (uint32_t i = 0; i < write_set.size(); ++i) {
    ...
    while (true) {
      ...
      context_change:
        ...
        if (reader_state == TXN_ACTIVE or not reader_end or
            reader_end > cstamp) {
            ...
            if (reader_xc->xct->is_read_mostly() and
              ...
              if (reader_xc->verify_owner(rxid)) {
                ...
                while (last_cstamp < reader_end) {
                  ...
                  if (volatile_read(reader_xc->state) == TXN_ABRTD or
  ...
  ALWAYS_ASSERT(my_sstamp and
                (my_sstamp & xid_context::sstamp_final_mark) == 0);
